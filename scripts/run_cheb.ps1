# =============================
# Chebyshev: gamma ∈ {10,20}, dt ∈ {1e-4,1e-5,1e-6}, tol ∈ {1e-11,1e-12}
# K=0 => без потолка, ряд обрезается по tol
# =============================

# locate exe
$root = Split-Path -Parent $PSScriptRoot
$candidates = @(
  (Join-Path $root 'build\Release\morse.exe'),
  (Join-Path $root 'Release\morse.exe'),
  (Join-Path $root 'build\morse.exe')
)
$exe = $null; foreach ($c in $candidates) { if (Test-Path $c) { $exe = $c; break } }
if (-not $exe) { Write-Error "Не найден morse.exe."; exit 1 }
Write-Host "Использую бинарник: $exe" -ForegroundColor Green

$gammas = @(10, 20)
$dts = @('1e-4','1e-5','1e-6')
$tols = @('1e-11','1e-12')

function Get-LogEvery([string]$dt) {
  switch ($dt) {
    '1e-4' { 10000 }
    '1e-5' { 50000 }
    '1e-6' { 100000 }
    default { 10000 }
  }
}

foreach ($g in $gammas) {
  foreach ($dt in $dts) {
    $log = Get-LogEvery $dt
    foreach ($tol in $tols) {
      Write-Host "=== Cheb: gamma=$g dt=$dt tol=$tol (log_every=$log) ===" -ForegroundColor Green
      & $exe --evolve cheb --gamma $g --dt $dt --tmax 10 --tol $tol --K 0 --log-every $log 
    }
  }
}
